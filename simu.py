...
if st.session_state.run:
    frame = 0
    # Käytetään while-silmukkaa, jotta sivu ei lataudu kokonaan uusiksi joka kerta
    while st.session_state.run:
        ...
        placeholder.vega_lite_chart(df_m, {
            'mark': {'type': 'line', 'clip': True, 'interpolate': 'linear'},
            'encoding': {
                'x': {'field': 'x', 'type': 'quantitative', 'scale': {'domain': [0, 6.28]}, 'axis': {'title': 'Aika (t)'}},
                'y': {'field': 'y', 'type': 'quantitative', 'scale': {'domain': [-2.5, 2.5]}, 'axis': {'title': 'Amplitudi'}},
                'color': {
                    'field': 'Aalto', 
                    'type': 'nominal',
                    'scale': {
        ...
        # Tarkistetaan pysäytysnappi (Streamlit vaatii tämän silmukan sisällä)
        # Jos käyttäjä muuttaa slideria tai painaa nappia, koodi keskeytyy automaattisesti
else:
